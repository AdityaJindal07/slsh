...
int main(void)
{
    ...
    assert(cmd_engine_init(NULL, 0U) == CMD_ENGINE_OK);
    assert(cmd_register(0x40U, "test_cmd", record_call) == CMD_ENGINE_OK);
    ...
    assert(command_register(0x41U, "another_test_cmd", record_call) == CMD_ENGINE_OK);
    assert(cmd_register(0x42U, "test_cmd", record_call) == CMD_ENGINE_ERR_DUPLICATE_COMMAND);
